...
# ==========================================
st.set_page_config(
    page_title="Dargeçit MEB | Proje & Performans Sistemi",
    page_icon="🏫",
    layout="wide",
...
DATA_FILE = "veritabani_v3.csv"

# Görselden alınan eksiksiz Dargeçit Okul Listesi
DARGEÇIT_OKULLARI = [
...
# Değiştirilmemesi İstenen Örnek Şablon
CEKIRDEK_SABLON = [
...
# Çoklu görev yapısı için genişletilmiş sütunlar
GEREKLI_SUTUNLAR = [
...
def ayar_yukle():
    if not os.path.exists(CONFIG_FILE):
        varsayilan = {
            ...
            "sablonlar": {"Gazi Matematik Şablonu (Örnek Proje/Performans Ölçeği)": CEKIRDEK_SABLON},
            "kullanicilar": {
    ...
    with open(CONFIG_FILE, "r", encoding="utf-8") as f:
        ...
        if "sablonlar" not in data or not data["sablonlar"]: 
            data["sablonlar"] = {"Gazi Matematik Şablonu (Örnek Proje/Performans Ölçeği)": CEKIRDEK_SABLON}
...
def eokul_sablon_olustur():
    sablon_df = pd.DataFrame(columns=['Okul No', 'Öğrenci Adı Soyadı', 'Sınıf', '1. Sınav Notu', '2. Sınav Notu'])
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
...
# ==========================================
# 4. YAPAY ZEKA BAĞLANTILARI (DEĞERLENDİRME & KARNE)
# ==========================================
...
# ==========================================
# 6. GİRİŞ VE KAYIT EKRANLARI (ÖĞRENCİ & ÖĞRETMEN)
# ==========================================
                ...
                if sonuclar.empty:
                    ...
                    for _, row in sonuclar.iterrows():
                        with st.expander(f"📌 {row['Gorev_Adi']} ({row['Ders']}) — Puan: {row['Toplam Puan']}"):
                            # Arayüzde şablon bulucu
                            kullanilan_sablon = CEKIRDEK_SABLON # Varsayılan
                            # Gerçekte hangi şablonla değerlendirildiyse onu çekmek ekstra mantık gerektirir, 
                            # JSON içindeki ID'lerden anlaşılabilir ama basitlik adına varsayılan gösteriliyor
                            html_k = toplu_karne_html_dosyasi_uret(pd.DataFrame([row]), row['Atanan_Ogretmen'], row['Ders'], kullanilan_sablon)
...
# ==========================================
# 7. ÖĞRETMEN VE İDARE YÖNETİM MASASI (DERLİ TOPLU)
# ==========================================
def yonetim_paneli(df, ayarlar):
    ...
    # Admin Tüm Veriyi, Öğretmen Sadece Kendi Okulu/Öğrencilerini Görür
    df_yetkili = df if rol == "admin" else df[(df['Okul'] == kb.get("okul")) & ((df['Atanan_Ogretmen'] == aktif_id) | (df['Atanan_Ogretmen'] == 'admin'))]

    # Sekmeleri Derli Toplu Gruplama
    if rol == "admin":
        sekme_basliklari = ["👥 Öğrenci & Görev İşlemleri", "🤖 AI Değerlendirme", "📊 Raporlar", "📝 E-Okul Karne", "⚙️ Sistem & Şablon Ayarları"]
    else:
        sekme_basliklari = ["👥 Öğrenci & Görev İşlemleri", "🤖 AI Değerlendirme", "📊 Raporlar", "📝 E-Okul Karne"]

    sekmeler = st.tabs(sekme_basliklari)

    # --- SEKME 1: ÖĞRENCİ VE GÖREV YÖNETİMİ (Ekle/Yükle/Sil Bir Arada) ---
    with sekmeler[0]:
        st.markdown("<div class='section-header'>Öğrenci Listesi ve Görev Tanımlama Alanı</div>", unsafe_allow_html=True)
        st.info("💡 **Önemli:** Öğrencilere birden fazla görev (örn: 1. Proje, 2. Performans) verebilirsiniz. Listeyi yüklediğinizde aynı öğrenci varsa çift kayıt olmaz, sadece o öğrenciye belirlediğiniz yeni görev eklenir.")
        ...
        with c_islem1:
            ...
            g_isim = st.text_input("Görevin Adı (Örn: Üslü Sayılar Projesi)")
            st.download_button("📄 Örnek Excel Şablonu", data=bos_sablon_olustur(), file_name="Ogrenci_Sablon.xlsx")
            
            uploaded_file = st.file_uploader("Öğrenci Listesini Yükle", type=['xlsx'])
            if uploaded_file and g_isim.strip() and st.button("🚀 Listeyi Yükle ve Görevleri Ata", use_container_width=True):
                try:
                    ...
                    for _, row in excel_df.iterrows():
                        # Çift Kayıt Engelleme: Öğrenci ve Görev Adı Eşleşmesi
                        kontrol = df[(df['Okul'] == h_okul) & (df['Okul No'] == row['Okul No']) & (df['Gorev_Adi'] == g_isim.strip())]
                        if kontrol.empty:
                            yeni = {c: None for c in GEREKLI_SUTUNLAR}
                            yeni.update({
                                'Okul': h_okul, 'Ekleyen': aktif_id, 'Atanan_Ogretmen': aktif_id,
                                'Ders': kb.get("brans","Genel"), 'Okul No': row['Okul No'],
    ...
    with sekmeler[1]:
        ...
        if df_yetkili.empty:
            ...
            # Şablon Seçimi - Matematik Şablonu Belirgin
    ...
    with sekmeler[2]:
        ...
        if not df_yetkili.empty:
            ...
            if c_rap2.button("🖨️ PDF/HTML Karneleri Üret", use_container_width=True):
                # Öğrencinin değerlendirildiği şablonu tam bilmek için aktif olanı varsayıyoruz
                h_cikti = toplu_karne_html_dosyasi_uret(df_y, kb.get("ad",""), kb.get("brans",""), CEKIRDEK_SABLON)
    ...
    with sekmeler[3]:
        ...
        k_dosya = st.file_uploader("Öğrenci Not Excel'ini Yükle", type=['xlsx'])
        if k_dosya:
        ...
        if "kdf" in st.session_state:
            ...
            obs = c_k1.text_area("Öğretmen Gözlemi (Opsiyonel)")
    ...
    # --- SEKME 5: SİSTEM VE ŞABLON AYARLARI (SADECE ADMİN) ---
    if rol == "admin":
        with sekmeler[4]:
            st.markdown("<div class='section-header'>Sistem ve Kriter Şablonları (Süper Yetkili)</div>", unsafe_allow_html=True)
            c_ay1, c_ay2 = st.columns(2)
            
            with c_ay1:
                st.markdown("#### 🏢 Okul / Sistem Ayarları")
                y_okul = st.text_input("Yeni Okul Ekle")
                if st.button("Ekle") and y_okul:
                    ayarlar["okullar"].append(y_okul)
                    ayar_kaydet(ayarlar); st.rerun()
                
                sil_okul = st.selectbox("Okul Sil", ["— Seçiniz —"] + ayarlar["okullar"])
                if st.button("Sil") and sil_okul != "— Seçiniz —":
                ...
                if ogrt:
                    sec_o = st.selectbox("Öğretmen", list(ogrt.keys()), format_func=lambda x: f"{ogrt[x]['ad']} ({x})")
                    y_sifre = st.text_input("Şifre Değiştir", value=ogrt[sec_o]['sifre'])
                    if st.button("Güncelle"): ayarlar["kullanicilar"][sec_o]['sifre'] = y_sifre; ayar_kaydet(ayarlar); st.success("Şifre Güncellendi")
                    if st.button("Öğretmeni Sil"): del ayarlar["kullanicilar"][sec_o]; ayar_kaydet(ayarlar); st.rerun()

            with c_ay2:
                st.markdown("#### 📐 Şablon Tasarlayıcı")
                st.info("Kriterlerin toplamı 100 puan olmalıdır.")
                ...
                sil_sablon = st.selectbox("Şablon Seç", list(ayarlar["sablonlar"].keys()))
                if st.button("🗑️ Şablonu Sil"):
                    if "Gazi" in sil_sablon: st.error("Varsayılan Şablon Silinemez!")
