...
CEKIRDEK_SABLON = [
    ...
    {"id": "k2", "baslik": "Düzen ve Tertip", "max": 15, "icon": "📐",
     "aciklama": "Ödevin temiz, okunaklı ve düzenli bir şekilde hazırlanmış olması."},
    ...
    {"id": "k5", "baslik": "Zamanında Teslim", "max": 15, "icon": "⏰",
     "aciklama": "Projenin belirtilen tarihte teslim edilmesi."}
]
...
Lütfen yukarıdaki ders notlarına ve özellikle 'Davranış' notuna (100 üzerinden bir akademik performans gibi analiz ederek)
bakarak öğrenciye hitap eden, pedagojik, 3-4 cümlelik motive edici bir dönem sonu karne görüşü üret. Türkçe."""
    payload = {
        "contents": [{"parts": [{"text": prompt}]}],
...
def yonetim_paneli(df, ayarlar):
    ...
    # ══════════════════════════════════════════════════
        ...
        if "kdf" in st.session_state and not st.session_state["kdf"].empty:
            ...
            if c_k1.button("🗑️ Bu Öğrenciyi Listeden Çıkar"):
            ...
            col_b1.download_button("📥 Tamamlanan Listeyi Excel Olarak İndir", data=out_k.getvalue(),
                                    file_name="E_Okul_Gorusleri.xlsx", use_container_width=True)
            if col_b2.button("🧹 Listeyi Temizle", type="primary", use_container_width=True):
    ...
    # ══════════════════════════════════════════════════
        ...
        if aktif_ayar == "sistem" and rol == "admin":
            ...
            st.markdown("<div class='section-header'>🏢 Okul Listesi Yönetimi</div>", unsafe_allow_html=True)
            y_okul_ekle = st.text_input("Yeni Okul Adı")
            if st.button("➕ Okul Ekle") and y_okul_ekle:
                ayarlar["okullar"].append(y_okul_ekle)
                ayar_kaydet(ayarlar)
                st.rerun()
            sil_okul = st.selectbox("Okul Sil", ["— Seçiniz —"] + sorted(ayarlar["okullar"]))
            if st.button("🗑️ Seçili Okulu Sil") and sil_okul != "— Seçiniz —":
                ayarlar["okullar"].remove(sil_okul)
                ayar_kaydet(ayarlar)
                st.rerun()
            st.markdown("**Kayıtlı Okullar:**")
            st.dataframe(pd.DataFrame({"Okul Adı": sorted(ayarlar["okullar"])}), hide_index=True, use_container_width=True)
